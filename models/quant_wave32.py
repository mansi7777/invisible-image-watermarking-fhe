...
class QuantWaveTF32x32(nn.Module):
    def __init__(self, image_size: Tuple[int], watermark_size: Tuple[int] = (16 * 16,), wavelet_type='haar'):
        ...
        self.watermark_preprocess = nn.Sequential(
            qnn.QuantLinear(self.watermark_size[0], 64),
            qnn.QuantIdentity(act_quant=Int8ActPerTensorFloat, return_quant_tensor=True),
            qnn.QuantReLU(),
            qnn.QuantLinear(64, 128),
            qnn.QuantIdentity(act_quant=Int8ActPerTensorFloat, return_quant_tensor=True),
            qnn.QuantReLU(),
            qnn.QuantLinear(128, 256),
            qnn.QuantIdentity(act_quant=Int8ActPerTensorFloat, return_quant_tensor=True),
            nn.Sigmoid(),
            qnn.QuantIdentity(act_quant=Int8ActPerTensorFloat, return_quant_tensor=True),
    ...
    def forward(self, image_input, watermark_input):
        image_input=self.quant_inp(image_input)
        watermark_input=self.quant_inp(watermark_input)
